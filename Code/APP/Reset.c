...
extern void ResetToCut(LogicParaDef *p);
extern void ManualDisBelt(LogicParaDef *p);
...
void Reset()
{
    ...
    if(GUR.RunStatus == D_RESET)
    {
		...
		case 4:
			if(AxSta(AxPickZ) == AXSTA_READY && I_Get(I_WeldCydUp)==ON) // && AxSta(AxWeld) == AXSTA_READY && AxSta(AxHit) == AXSTA_READY)
			{
				GUR.HitReady = 1;
...
//				AxAbsUnit(AxWeld,10,GPOS.WeldReady);
        ...
        case 7:
				...
				AxSta(AxPickRF) == AXSTA_READY)
			{
				...
				LT.ManualDisBeltTask.execute = 1;
				LT.ResetToCutTask.execute = 1;
				REST_TASK.step = 8;
			}
            break;
			
		case 8 :
			if(LT.ManualDisBeltTask.execute == 0 &&LT.ResetToCutTask.execute == 0)
			{
				PARAINIT(REST_TASK);
				GSW.ClearAlarm = 1;
				GUR.HaveToReset = 0;
